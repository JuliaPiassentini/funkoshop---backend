...
const getOne = async (params) => {
        try {
        const [rows] = await conn.query('SELECT product.*,licence.licence_name, licence.licence_description FROM product LEFT JOIN licence ON product.licence_id = licence.licence_id WHERE ?;', params);/*Me trae toda la info del producto y le agrega el nombre de la licencia coincidente con el licence _id que tiene product .Filtramos por el param que determinemos en el controlador*/
        return rows;
